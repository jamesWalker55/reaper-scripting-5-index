...
function ____exports.generateContainerFxidx(self, track, allidx)
    assert(#allidx > 0, "container index must be at least length 1")
...
function TrackFX.prototype.GetParamEx(self, param)
    local rv, min, max, mid = reaper.TrackFX_GetParamEx(self.track, self.fxidx, param)
    if min == nil then
        error("failed to get param value")
    end
    return {rv, min, max, mid}
end
function TrackFX.prototype.SetParam(self, param, value)
    return reaper.TrackFX_SetParam(self.track, self.fxidx, param, value)
end
...
function TrackFX.prototype.rename(self, name)
    reaper.TrackFX_SetNamedConfigParm(self.track, self.fxidx, "renamed_name", name)
end
...
function TakeFX.prototype.GetParamEx(self, param)
    local rv, min, max, mid = reaper.TakeFX_GetParamEx(self.take, self.fxidx, param)
    if min == nil then
        error("failed to get param value")
    end
    return {rv, min, max, mid}
end
function TakeFX.prototype.SetParam(self, param, value)
    return reaper.TakeFX_SetParam(self.take, self.fxidx, param, value)
end
...
function TakeFX.prototype.rename(self, name)
    reaper.TakeFX_SetNamedConfigParm(self.take, self.fxidx, "renamed_name", name)
end
...
function FXParam.prototype.getValue(self)
    local cur, min, max, mid = table.unpack(self.fx:GetParamEx(self.param))
    return {cur = cur, min = min, max = max, mid = mid}
end
function FXParam.prototype.setValue(self, value)
    return self.fx:SetParam(self.param, value)
end
