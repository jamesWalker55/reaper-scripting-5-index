    ...
    microUILoop(
        ...
        function()
                ...
                do
                    ...
                    for ____, track in __TS__Iterator(Track:iterAll()) do
                        for ____, item in __TS__Iterator(track:iterItems()) do
                            local itemPos = checkItemEnds and item:getPosition() + item:getLength() or (checkItemSnapOffset and item:getPosition() + item:getSnapOffset() or item:getPosition())
            ...
            if ctx:beginWindow("Demo Window", {x = 0, y = 0, w = 0, h = 0}, Option.NoResize | Option.NoTitle | Option.NoClose) then
                ...
                if ctx:header("Detected offset positions", Option.Expanded) then
        ...
        function()
            for ____, track in __TS__Iterator(Track:iterAll()) do
                for ____, item in __TS__Iterator(track:iterItems()) do
                    local currentColor = item:getColor()
