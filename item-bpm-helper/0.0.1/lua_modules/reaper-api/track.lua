...
__TS__SetDescriptor(
    ...
    {
        get = function(self)
            return reaper.GetMediaItemInfo_Value(self.obj, "B_LOOPSRC") ~= 0
        end,
        set = function(self, x)
            local rv = reaper.SetMediaItemInfo_Value(self.obj, "B_LOOPSRC", x and 1 or 0)
            if not rv then
                error(
                    __TS__New(Error, "failed to set item loop"),
                    0
                )
            end
        end
...
__TS__SetDescriptor(
    ...
    {
        get = function(self)
            return reaper.IsMediaItemSelected(self.obj)
        end,
        set = function(self, x)
            reaper.SetMediaItemSelected(self.obj, x)
        end
    },
    true
)
...
function Take.prototype.getAllFx(self)
    local count = self:getFxCount()
    local result = {}
    do
        local i = 0
        while i < count do
            result[#result + 1] = __TS__New(FX, {take = self.obj}, i)
            i = i + 1
        end
    end
    return result
end
function Take.prototype.getFx(self, idx)
    return __TS__New(FX, {take = self.obj}, idx)
end
...
do
    ...
    function TrackRouting.getAudioInfo(self, track, category, idx)
        ...
        local channelCount
        repeat
            local ____switch210 = I_SRCCHAN >> 10
    ...
    function TrackRouting.getParentMidiInfo(self, track)
        ...
        repeat
            local ____switch217 = I_MIDI_CTL_CHAN
            local ____cond217 = ____switch217 == 16
            if ____cond217 then
